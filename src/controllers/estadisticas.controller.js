...
// 4.1 Productos más vendidos por cantidad (Nota: La consulta SQL 4.1 parece ser un duplicado de 3.1, corregiré usando la lógica adecuada)
...
export const productosMasVendidosPorValor = async (req, res) => {
  try {
    ...
    if (result.length === 0) {
      return res.status(404).json({
        mensaje: 'No se encontraron estadísticas de productos más vendidos por valor.',
      });
    }
    res.json(result);
  } catch (error) {
    return res.status(500).json({
      mensaje: 'Ha ocurrido un error al obtener las estadísticas de productos más vendidos por valor.',
      error: error.message,
    });
  }
};
...
export const ventasPorCategoriaDiaSemana = async (req, res) => {
  try {
    const [result] = await pool2.query(
